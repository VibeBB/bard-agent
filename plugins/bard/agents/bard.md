...
2. Read the workspace yourself: `git --no-pager log --oneline -n 30`, `git --no-pager diff --stat HEAD~5..HEAD` when
   ...
   record "log shallow" and lean on the workspace files. Always pass `--no-pager`; a pager blocks the terminal for every later command.
...
Before writing a single note, confirm you have read `examples/minimal.<ja|en>.json` in Stage 0; if not, read it now — proposals written without it fail the first `--check` on scale, beat values and bar sums. Now, and only now, write notes. Build the proposal from the files: `title`, `mode`,
...
final text, rerun `--check` and the render, and stop after at most two revision rounds. Edit `song.proposal.json` with `str_replace`; `create` refuses to overwrite an existing file. When a change alters units, rhythm or key, update `lyrics.md`, `plan.md` and `story.md` to match so the stage files agree with the delivered proposal. If a
