...
`find` or `grep` on the artifacts are allowed.
"""
...
def _is_artifact_write(payload: dict[str, Any]) -> bool:
    ...
    if isinstance(action, str):
        ...
        if action in WRITE_ACTIONS:
            return True
    return any(
        key in tool_input for key in ("file_text", "new_str", "content", "insert_text")
    )
...
def main() -> int:
    ...
    if target is not None:
        print(
            f"song artifacts may not be written through the terminal: {target}",
            file=sys.stderr,
        )
        return 2
    return 0
