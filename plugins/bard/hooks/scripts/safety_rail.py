...
def _denied_rm(args: list[str]) -> bool:
    ...
    for arg in args:
        if arg.startswith("--"):
            ...
            operands.append(arg)
    return (
        recursive and force and any(_is_root_or_home(operand) for operand in operands)
    )
...
def _denied_git(args: list[str]) -> str | None:
    ...
    if subcommand == "commit" and any(
        flag in ("--amend", "--no-verify", "-n") for flag in flags
    ):
...
def _denied_segment(segment: list[str]) -> str | None:
    ...
    if name == "dd" and any(
        arg.startswith("of=") and DEVICE_TARGET.match(arg[3:]) for arg in args
    ):
