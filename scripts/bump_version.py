"""Bump the bard plugin version across every version-bearing file.

Updates plugins/bard/.plugin/plugin.json, pyproject.toml, both SKILL.md
frontmatter lines and the bard-agent package entry in uv.lock. Fails closed
...
UV_LOCK = "uv.lock"
UV_LOCK_RE = re.compile(r'(?m)^(name = "bard-agent"\nversion = )"([^"]+)"')
...
def _apply(root: Path, old: str, new: str) -> None:
    for rel, pattern in _PATTERNS.items():
        ...
        replaced = pattern.sub(
            lambda m, v=new: m.group(0).replace(m.group(1), v), text, count=1
        )
    ...
    if lock.is_file():
        ...
        if n != 1:
            raise BumpError(f"{UV_LOCK}: bard-agent package entry not found")
    ...
    try:
        ...
        if args.set_version:
            ...
            if not _gt(new, current):
                raise BumpError(
                    f"--set '{new}' must be greater than current '{current}'"
                )
        else:
