...
def _default_fetch_json(url: str) -> Any:
    request = Request(url, headers={"User-Agent": "bard-dep-check"})
...
def dependency_names(data: dict[str, Any]) -> list[str]:
    ...
    for dep in project.get("dependencies", []):
        add(dep)
    groups = _dict(
        data.get("dependency-groups", {}), "dependency-groups is not an object"
    )
...
def _uv_source_names(data: dict[str, Any]) -> set[str]:
    ...
    sources = _dict(
        data.get("tool", {}).get("uv", {}).get("sources", {}), "tool.uv.sources"
    )
    ...
    *,
    ...
    for line in output.splitlines():
        ...
        for pattern, kind in patterns:
            ...
            statuses.append(
                DependencyStatus(
                    "pypi-lock", name, current, latest, "uv.lock", True, note
                )
            )
            break
    return statuses
...
_DOCKERFILES = ["bard-tools.Dockerfile"]
...
_DOCKER_ARG_UPSTREAMS: dict[str, tuple[str, str]] = {
...
def check_docker_args(
    ...
    for arg, (repo, strip) in _DOCKER_ARG_UPSTREAMS.items():
        ...
        if current is None:
            statuses.append(
                DependencyStatus(
                    "docker-arg", arg, "-", "?", _DOCKERFILES[0], False, "ARG missing"
                )
            )
            continue
        latest = _github_latest_tag(repo, list_remote_tags)
...
def _supported_docker_base(image: str, tag: str) -> bool:
    return (
        image == "ubuntu"
        and re.fullmatch(r"\d{2}\.\d{2}", tag) is not None
        or image == "debian"
        and re.fullmatch(r"\d+-slim", tag) is not None
    )


def _latest_docker_base_tag(
    fetch_json: FetchJson, image: str, current: str
) -> str | None:
    if image == "ubuntu":
        tags = [
            t
...
def load_deferrals(repo_root: Path) -> list[DependencyDeferral]:
    ...
    for raw_deferral in cast(list[Any], raw_deferrals):
        ...
        try:
            ...
            raise ValueError(
                f"dependency deferral has invalid review_by: {review_by}"
            ) from exc
        ...
        if status.outdated:
            for deferral in deferrals:
                if (
                    ...
                    note = (
                        f"deferred until {deferral.review_by.isoformat()}: "
                        f"{deferral.reason}"
                    )
                    if status.note:
                        note = f"{status.note}; {note}"
                    replacement = replace(
                        status, outdated=False, note=note, deferred=True
                    )
                    break
...
def render_markdown(statuses: list[DependencyStatus]) -> str:
    ...
    for surface, label in labels.items():
        ...
        for status in ordered_statuses:
            ...
            if status.outdated:
                ...
                state = "up to date"
            lines.append(
                f"| {status.name} | {status.current} | {latest} | {state} |"
                f" {status.source} |"
            )
        lines.append("")
    ...
    lines.append(
        f"update candidates: {outdated_count}"
        if outdated_count
        else "No update candidates."
    )
    return "\n".join(lines) + "\n"
